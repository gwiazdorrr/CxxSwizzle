		...
		if (NOT line MATCHES "^#define.*")
			if (line MATCHES "([A-Za-z0-9_]+)[ \t]*\\[([A-Za-z0-9_ \t]*)\\][ \t]*([A-Za-z0-9_]+)")
				...
				if ("${CMAKE_MATCH_2}" STREQUAL "")
                    if (line MATCHES ^"[ \t]")
                        string(REGEX REPLACE "([A-Za-z0-9_]+)[ \t]*\\[([A-Za-z0-9_ \t]*)\\][ \t]*([A-Za-z0-9_]+)" "CXXSWIZZLE_ARRAY(\\1) \\3" line "${line}")
                    else()
                        string(REGEX REPLACE "([A-Za-z0-9_]+)[ \t]*\\[([A-Za-z0-9_ \t]*)\\][ \t]*([A-Za-z0-9_]+)" "CXXSWIZZLE_ARRAY_FIELD(\\1) \\3" line "${line}")
                    endif()
				else()
					string(REGEX REPLACE     "([A-Za-z0-9_]+)[ \t]*\\[([A-Za-z0-9_ \t]*)\\][ \t]*([A-Za-z0-9_]+)" "CXXSWIZZLE_ARRAY_N(\\1, \\2) \\3" line "${line}")
				endif()
			elseif (line MATCHES "([A-Za-z0-9_]+)[ \t]+([A-Za-z0-9_]+)[ \t]*\\[([A-Za-z0-9_ \t]*)\\]")
				...
				if (NOT "${CMAKE_MATCH_1}" STREQUAL "return")
					if ("${CMAKE_MATCH_3}" STREQUAL "")
                        if (line MATCHES ^"[ \t]")
                            string(REGEX REPLACE "([A-Za-z0-9_]+)[ \t]+([A-Za-z0-9_]+)[ \t]*\\[([A-Za-z0-9_ \t]*)\\]" "CXXSWIZZLE_ARRAY(\\1) \\2" line "${line}")
                        else()
                            string(REGEX REPLACE "([A-Za-z0-9_]+)[ \t]+([A-Za-z0-9_]+)[ \t]*\\[([A-Za-z0-9_ \t]*)\\]" "CXXSWIZZLE_ARRAY_FIELD(\\1) \\2" line "${line}")
                        endif()
					else()
						string(REGEX REPLACE     "([A-Za-z0-9_]+)[ \t]+([A-Za-z0-9_]+)[ \t]*\\[([A-Za-z0-9_ \t]*)\\]" "CXXSWIZZLE_ARRAY_N(\\1, \\3) \\2" line "${line}")
					endif()
				endif()
			endif()
