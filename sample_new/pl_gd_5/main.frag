...
float ShadowOcclusion(vec3 MarchOrigin, vec3 MarchEnd)
{
    ...
    for (int i = 0; i < RaymarchStepCount; ++i)
    {
        ...
        Occlusion *= step(LocalDisplacement, RayAltitude);
    }
    return Occlusion;
}
...
vec3 ShadeReference(vec3 LightPos, float LightIntensity, vec3 Albedo, vec3 PixelWorldPos, vec3 PixelWorldNormal, float DisplacementSample)
{
    ...
    vec3 DiffuseLighting = LightIntensity * Attenuation * Occlusion * OrenNayarDiffuse(L, N, V, 0.3, Albedo);
...
void mainImage(out vec4 FragColour, in vec2 FragCoord)
{
    ...
    vec3 PixelWorldPos = Unproject(UV, DisplacementSample, PixelWorldNormal);
