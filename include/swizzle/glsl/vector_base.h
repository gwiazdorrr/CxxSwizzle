...
namespace swizzle
{
    ...
    {        
        ...
        public:
            ...
            using number_scalar_arg = only_if< build_info::is_number, scalar_arg >;
            using number_vector_arg = only_if< build_info::is_number, this_type_arg >;
            
            using float_scalar_arg = only_if< build_info::is_floating_point, scalar_arg >;
            using float_vector_arg = only_if< build_info::is_floating_point, this_type_arg >;

            using bool_type = typename build_info::bool_type;
            using bool_vector_type = vector_<bool_type, Index...>;
            using bool_scalar_arg = only_if< build_info::is_bool, this_type_arg >;
            using bool_vector_arg = only_if< build_info::is_bool, this_type_arg >;
            ...
            inline vector_(only_if<num_of_components == 1, scalar_arg> s)
            {
                at(0) = s;
            }

            //! Needed to differentiate from explicit one arg constructor
            inline vector_(only_if<num_of_components == 1, scalar_type&&> s)
            {
                at(0) = s;
            }

            //! For vectors bigger than 1 conversion from scalar should be explicit.
            inline explicit vector_(only_if<num_of_components != 1, scalar_arg> s)
            {
                ((at(Index) = s), ...);
            }
            ...
            template <class T0, class = std::enable_if_t<(num_of_components <= detail::get_total_size_v<T0>)> >
            ...
            template <class T0, class T1, class... T,
                ...
                (num_of_components <= detail::get_total_size_v<T0, T1, T...>) &&
                !(num_of_components <= detail::get_total_size_v<T0, T1, T...> -detail::get_total_size_v< detail::last_t<T0, T1, T...> >)
                >
            >
            ...
            inline internal_scalar_type& at(only_if<are_scalar_types_same, size_t> i)
            {
                return data[i];
            }
            inline const internal_scalar_type& at(only_if<are_scalar_types_same, size_t> i) const
            {
                return data[i];
            }

            inline internal_scalar_type& at_raw(size_t i)
            {
                return data[i];
            }
            inline const internal_scalar_type& at_raw(size_t i) const
            {
                return data[i];
            }

            //! These are chosen when internal_scalar_type and outside visible scalar type are not same.
            inline scalar_type& at(only_if<!are_scalar_types_same, size_t> i)
            {
                return *reinterpret_cast<scalar_type*>(&data[i]);
            }

            inline const scalar_type& at(only_if<!are_scalar_types_same, size_t> i) const
            {
            ...
            inline operator only_if<num_of_components == 1, scalar_type>() const
            {
                return at(0);
            }
        ...
        private:
            ...
            void construct(T0&& t0, Tail&&... tail)
            {
                ...
                construct<Offset + detail::get_total_size_v<T0>>(std::forward<Tail>(tail)...);
            }
            ...
            void compose(const vector_<OtherScalarType, OtherIndex...>& v)
            {
                ...
                compose_impl2<Index>(v, detail::take_n<limit, OtherIndex...> {});
            }

            template <size_t Offset, typename DataTypeA, typename DataTypeB, size_t... DataIndex>
            inline void compose_impl(DataTypeA& dst, const DataTypeB& src, std::index_sequence<DataIndex...>)
            {
                ((dst[DataIndex + Offset] = src[DataIndex]), ...);
            }

            template <size_t Offset, typename DataTypeB, size_t... DataIndex>
            inline void compose_impl2(const DataTypeB& src, std::index_sequence<DataIndex...>)
            {
        ...
        public:
            ...
            //static this_type call_intBitsToFloat(only_if<build_info::is_floating_point, int_vector_type> value)
            //{
            //    this_type result;
            //    ((bitcast(value.data[Index], &result.data[Index])), ...);
            //    return result;
            ...
            //static uint_vector_type call_uintBitsToFloat(only_if<build_info::is_floating_point, uint_vector_type> value)
            //{
            //    this_type result;
            //    ((bitcast(value.data[Index], &result.data[Index])), ...);
            //    return result;
            ...
            static typename only_if<num_of_components == 3, this_type> call_cross(float_vector_arg x, float_vector_arg y)
            {
            ...
            inline only_if<build_info::is_number, this_type> operator-() const
            {
                this_type result;
    ...
    {
        template <typename ScalarType, size_t... Index>
        struct get_vector_type_impl< ::swizzle::glsl::vector_<ScalarType, Index...> >
        {
            typedef ::swizzle::glsl::vector_<ScalarType, Index...> type;
        };
    }
    ...
    namespace glsl
    {
        ...
        {
            {
                //detail::get_vector_type_impl<vector_<float, 0, 1, 2, 3>>::type aaaaa;
                detail::get_vector_type<vector<float, 4>, float, float>::type aaaaab;
                vector<float, 4> aaa(1.0f);
                vector<float, 4> bbb(666.0f);
                std::cout << aaa;
                //aaa.compose<3>(ccc);
                aaa += bbb;
                //aaa.compose<1>(bbb);
            ...
            {
                vector<bool, 4> aaa(false);
                vector<bool, 4> bbb(true);
