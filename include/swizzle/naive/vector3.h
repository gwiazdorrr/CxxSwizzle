#ifndef HEADER_GUARD_SWIZZLE_NAIVE_VECTOR3
#define HEADER_GUARD_SWIZZLE_NAIVE_VECTOR3
...
namespace swizzle
{
    ...
    {
        template <class T>
        class vector3 : public vector_base< vector3<T>, T >
        {
        public:
            //! Component type
        ...
        public:
            ...
            vector_type() : x( scalar_type() ), y( scalar_type() ), z( scalar_type() )
            {}

            explicit vector_type(const scalar_type& src) : x(src), y(src), z(src)
            {}

            vector_type(const vector_type& src) : x(src.x), y(src.y), z(src.z)
            {}

            vector_type(const scalar_type& x, const scalar_type& y, const scalar_type& z) : x(x), y(y), z(z)
            {}
...
#endif  HEADER_GUARD_SWIZZLE_NAIVE_VECTOR3
